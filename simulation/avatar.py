...
class Avatar(abstract_avatar):
    def __init__(self, args, avatar_id, init_property, init_statistic, init_personality=None):
        super().__init__(args, avatar_id)

        self.parse_init_property(init_property)
        self.parse_init_statistic(init_statistic)
        self.parse_init_personality(init_personality)
...
# activity_dict
        diversity_dict = {  1:"An Exceedingly Discerning Selective Viewer who watches movies with a level of selectivity that borders on exclusivity. The movie choices are meticulously curated to match personal taste, leaving no room for even a hint of variety.",
                            2:"A Niche Explorer who explores different genres and mostly sticks to preferred movie types. But explores different movies than preffered when the film sounds interesting",  
    ...
    def make_next_decision(self, remember=False, current_page=None):
        ...
        sys_prompt = ("You excel at role-playing. Picture yourself as a real time user exploring a movie recommendation system to look at preffered movies. You have the following social traits: " \
                    +f"\nYour activity trait is described as: {self.activity_dsc}"
                    +f"\nNow you are in Page {current_page}. You may get tired with the increase of the pages you have browsed."
