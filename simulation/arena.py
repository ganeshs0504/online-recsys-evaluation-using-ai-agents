...
class Arena(abstract_arena):
    ...
    def initialize_custom_avatars(self):
        super().initialize_all_avatars()
        # self.simulated_avatars_id = 1 # add number of lines from the custom persona.csv file

        

        self.persona_df = pd.read_csv(f"datasets/{self.dataset}/simulation/custom_persona.csv")
        self.user_statistic = pd.read_csv(f'datasets/{self.dataset}/simulation/custom_user_statistic.csv', index_col=0)
        self.user_personality = pd.read_csv(f'datasets/{self.dataset}/simulation/custom_personality_statistic.csv', index_col=0)
        ...
        for avatar_id in self.simulated_avatars_id:
            self.avatars[avatar_id] = Avatar(self.args, avatar_id, self.persona_df.iloc[avatar_id], self.user_statistic.iloc[avatar_id], self.user_personality.iloc[avatar_id])
    ...
    async def async_simulate_one_avatar(self, avatar_id, loop, executor):
        ...
        interview_response = avatar_.response_to_question("Answer the following questions in the format of - RATING: [integer between 1 and 10]; REASON: [explanation for the rating]"
                                                          +"\nQ1: Do you feel satisfied with the recommender system you have just interacted? Rate it from 1 - 10 and give explanation."
                                                          +"\nQ2: On a scale of 1 to 10, how much did the recommender system positvly surprise you. This metric calculates how inclined are you to watch the recommended movie even though it didnt align with the personal taste. THe motivation can be from the overall quality of the recommender system or the movie choice in itself")
    ...
    def save_results(self):
        ...
        with open(self.storage_base_path + "/ctr.pkl", 'wb') as f:
            pickle.dump(average_click_rate, f)
